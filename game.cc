...
void Game::FilterIntersections() {  
  for (int i = 0; i < opponents_.size(); i++) {    
    if (opponents_[i].GetIsActive() && my_player_.GetIsActive() && my_player_.IntersectsWith(opponents_[i])) { 
      my_player_.SetIsActive(false);    
      } else {      
  for (int j = 0; j < player_projectiles_.size(); j++) {        
    if (opponents_[i].GetIsActive() && player_projectiles_[j].GetIsActive() && 
...
void Game::UpdateScreen() {  
  game_screen_.DrawRectangle(0, 0, game_screen_.GetWidth(),game_screen_.GetHeight(), 
